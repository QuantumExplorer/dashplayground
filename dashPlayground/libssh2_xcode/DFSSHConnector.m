...
static int waitsocket(int socket_fd, LIBSSH2_SESSION *session) {
    ...
    rc_connector = select(socket_fd + 1, readfd, writefd, NULL, &timeout);
...
-(int) connectInit:(DFSSHServer*)server {
   ...
   //     hostaddr = inet_addr([[server hostname]UTF8String]);
    ...
    hostaddr = inet_addr([resolvedHost UTF8String]);
        ...
        while ((rc_connector = libssh2_session_startup([server session], [server sock])) == LIBSSH2_ERROR_EAGAIN);
        //while ((rc_connector = libssh2_session_handshake([server session], [server sock])) == LIBSSH2_ERROR_EAGAIN);
        if (rc_connector) {
            NSLog(@"Failure establishing SSH session: %lu", rc_connector);
            server.session=nil;
            return -1;
        }
...
-(int) connectPublicKey:(DFSSHServer*)server {
    ...
    while ((rc_connector = libssh2_userauth_publickey_fromfile([server session], 
                                                     [[server username] UTF8String], 
                                                     [[server keypub] UTF8String], 
                                                     [[server key] UTF8String], 
                                                     [[server password] UTF8String])) == LIBSSH2_ERROR_EAGAIN);
    if (rc_connector == LIBSSH2_ERROR_AUTHENTICATION_FAILED) {
        NSLog(@"Authentication by public key failed");
        return 1;
    } else if (rc_connector) {
        NSLog(@"Authentication by public key failed for random reason");
        return 1;
    }
...
-(int) connectPassword:(DFSSHServer*)server {
    ...
    if ( strlen([[server password]UTF8String]) != 0 ) {
            ...
            while ((rc_connector = libssh2_userauth_password([server session], [[server username]UTF8String], [[server password]UTF8String]) ) == LIBSSH2_ERROR_EAGAIN);
            if (rc_connector) {
...
-(int) connectKeyboard_Interactive:(DFSSHServer*)server {
    ...
    passwordFunc();    
    libssh2_session_set_blocking([server session], 1);
    int rc_connector = (libssh2_userauth_keyboard_interactive([server session], [[server username]UTF8String], &kb_int));
    
    
    if  (rc_connector > 0) {
        printf("\tAuthentication by keyboard-interactive failed!\n");
        return 1;
    } else if (rc_connector < 0) {
        printf("\tAuthentication by keyboard-interactive failed!\n");
        return 1;
    }
    libssh2_session_set_blocking([server session], 0);
    if (rc_connector == LIBSSH2_ERROR_EAGAIN) {
        printf("\tAuthentication by keyboard-interactive failed!\n");
    }
    [server setConnected:TRUE];   
    return 0;
